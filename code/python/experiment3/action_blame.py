...
"""
TODO: make functions expect R_table, rather than just R and recreate every time. This
will make code much more stable and predictable
"""
...
def expected_rewards_multi_ZD(R_table, probs):
	...
	for count, agent in enumerate(np.array(['action_a','action_b','action_c'])):#range(n):
		prob_mat.loc[:,agent] = prob_mat.loc[:,agent].map({1: probs[count], 0: 1-probs[count]})

	# getting overall probability of each situation by multiplying probability of each action
	df['prob'] = prob_mat[['action_a','action_b','action_c']].apply(np.prod,axis=1)
...
return:
	pivotality 		float 			pivotality to optimal rewards
"""
def pivotality_zd(R_table, choices, agent, opt=True, graded=False):
	# if it's not possible to get reward, blame is 0
	if np.sum(R_table['reward']) == 0: 
		return 0

	if opt:
		# maximum attainable reward
		max_reward = R_table['reward'].values.max()
		# finding the rows in the truth table that produce an *optimal* reward
		best_choices = R_table[R_table['reward'].values == max_reward]
	else:
	...
	changes = best_choices[['action_a','action_b','action_c']].apply(lambda x: x - choices, axis=1)
	...
	if graded == False:
		...
		# comparing rewards table to actual choices made in the situation
		choice_compare  = R_table.iloc[:,:n_agents].apply(lambda x: x == choices, axis=1)
		# finding the row in the rewards table that corresponds to all actual choices being made
		actual_case = R_table[choice_compare.apply(sum, axis=1) == n_agents]
		actual_reward = actual_case['reward']
		# weighting pivotality by the reward missed out on
		graded_piv = (max_reward - actual_reward) * np.max(changes['pivotality'])
		return graded_piv
...
"""
def rationality_pivotality_opt(R_table, k, beta, choices, agent, w, graded_piv=False):
	# probability that agent clears trees
	p_trees = probability_k_zd(R_table,k,beta)[agent]
	
	# pivotality of agent
	pivotality = pivotality_zd(R_table,choices,agent, opt=True, graded=graded_piv)
