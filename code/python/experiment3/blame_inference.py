...
"""
# computing the prior probability distribution when the optimal is not possible
prior = situations_df(3,3,2,3) 
...
def model_kld(dfs, 
	...
	# wanna save parameters and kl-divergences, can re-run once we know most diagnostic cases
	store_posts = []
	...
	for combo in list(itertools.combinations(range(len(store_posts)),2)): 
		...
		#kl_divs.append(entropy(store_posts[combo[1]].values))
		#kl_divs.append(kl_divergence(store_posts[combo[0]].values, store_posts[combo[1]].values))
...
a_rationality=situations_df(3,3,2,3, w=1)
a_pivotality=situations_df(3,3,2,3, w=0)
a_mixture=situations_df(3,3,2,3, w=.6) 
dfs = [a_rationality, a_mixture, a_pivotality]
# removing things to find the uncertainty over that value
ee_a = situations_looper(dfs=dfs, decision_beta=.1)
pp.sort_values('mean_kld', ascending=False)[:40]
pp.to_csv('pp.csv')

# only getting cases where there's 1 or 2 unknowns
pp2 = pp[pp.isnull().sum(axis=1).isin([1,2])]   
"""

"""
a_pivotality  = situations_df(n_trees=3, n_strengths=3, n_actions=2, n_people=3, w=0, rationality_beta=.5, k=1)
a_rationality = situations_df(n_trees=3, n_strengths=3, n_actions=2, n_people=3, w=1, rationality_beta=.5, k=1)
trees = 3
strength_a = None
strength_b = 1
strength_c = 1
...
scale=.1
print('pivotality')
print(situation_inf(a_pivotality, decision_beta=decision_beta, scale=scale, trees=trees, strength_a=strength_a, strength_b=strength_b, strength_c=strength_c, action_a=action_a, action_b=action_b, action_c=action_c, reward=reward, blame_a=blame_a, blame_b=blame_b, blame_c=blame_c) )
print('rationality')
print(situation_inf(a_rationality, decision_beta=decision_beta, scale=scale, trees=trees, strength_a=strength_a, strength_b=strength_b, strength_c=strength_c, action_a=action_a, action_b=action_b, action_c=action_c, reward=reward, blame_a=blame_a, blame_b=blame_b, blame_c=blame_c) )
"""
